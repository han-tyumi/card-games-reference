...
**57 entries, checked 2026-08-01** — pagat and Wikipedia, fetched and read
...
**3 entries, checked 2026-08-03** — the solitaires and light games the
...
`beggar-my-neighbour`, `forty-thieves`.
...
- Paraphrase that swaps the vocabulary scores like independent writing. Measured
  ...
  rewording is not.
- 9 entries are still resting on **one** source rather than two, because pagat
  carries few solitaires: `freecell`, `golf-multiplayer`, `klondike`, `koi-koi`,
  `mau-mau`, `pyramid`, `speed`, `spider`, `tripeaks`. Several list further
  sources in `sources_consulted` that were not pulled. The 2026-08-03 pass above
