...
**58 entries, checked 2026-08-01** — pagat and Wikipedia, fetched and read
...
**2 entries, checked 2026-08-03** — the solitaires and light games the
...
- Paraphrase that swaps the vocabulary scores like independent writing. Measured
  ...
  rewording is not.
- 10 entries are still resting on **one** source rather than two, because pagat
  carries few solitaires: `forty-thieves`, `freecell`, `golf-multiplayer`,
  `klondike`, `koi-koi`, `mau-mau`, `pyramid`, `speed`, `spider`, `tripeaks`.
  Several list further sources in `sources_consulted` that were not pulled. The
  2026-08-03 pass above is working through them.
