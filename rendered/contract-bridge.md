...
Use the full 52-card pack with jokers removed. Cards rank ace high down to the two, and the four suits are equal until the auction settles which of them, if any, is trumps. The player on the dealer's left shuffles and the player on the dealer's right cuts. Deal the entire pack one card at a time clockwise, beginning with the player on the dealer's left and ending with the dealer, thirteen to each player. Where a second pack is in use, the dealer's partner shuffles it while the hand is going on and sets it down on their own right, which is exactly where the next dealer sits, so the following deal is ready the moment this one finishes.
...
An established revoke is not replayed. The hand is finished out and tricks are handed over afterwards. If the revoking player won the trick they revoked on, that trick goes across, and one more with it if the offending side won any trick later in the hand. If the revoking player did not win that trick, a single trick goes across provided the offending side won that trick or a later one. Nothing at all is transferred where the offending side took no trick from the revoke onwards, where the revoke happened on the twelfth trick, or where both sides revoked on the same deal. Should the transfer still leave the innocent side worse off than an honest hand would have, the score is adjusted to make up the rest.
