...
The pairing rule is where hands are won. Read your grid as three columns of two. When both cards of a column are the same rank, that column scores nothing whatever they are worth, so a pair of queens costs you nothing instead of twenty, and chasing a second queen to sit under a first is often the strongest thing you can do with a turn. Matches across a row or on a diagonal do nothing at all; only the vertical pairs cancel. The rule cuts both ways, though, and beginners get caught by it: pairing your 2s turns minus four into zero, so the cards you least want to match are the ones already helping you.
