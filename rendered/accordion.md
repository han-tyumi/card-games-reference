...
Only the exposed card of a pile counts. A pile may be moved onto the pile immediately to its left, or onto the pile three places to its left, and only when the two exposed cards share a suit or share a rank. Nothing else is ever legal. Colour is irrelevant and so is rank order: a 4 does not go on a 5 for being next to it in sequence, it goes on another 4 for being a 4, or on any other club for being a club. Nothing ever moves rightwards, and the left-hand end of the line is a dead end by construction: the first pile has nothing to its left and can never move at all, while the second and third have a neighbour to slide onto but no pile three places back, so a leap is not available to them.
...
Gaps are what catch beginners out. When a pile leaves the middle of the line, everything to its right shifts one place left to close up, so the line stays continuous and is one pile shorter than before. Every position downstream of a move acquires a new left-hand neighbour and a new third-to-the-left. Distances are always counted on the line as it stands, never on the line as it was dealt, and forgetting that is the commonest way to talk yourself into an illegal move.
