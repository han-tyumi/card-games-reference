<!-- Generated by scripts/render-markdown.ts from games/crazy-eights.json. Do not edit by hand. -->
...
**Crazy Eights Countdown** — Each player is given a personal countdown number that starts at eight. The card of that rank is that player's own wild card, so at the beginning everyone's wild card is the eight. Playing your wild card lets you name a suit and knocks your countdown down by one, so your wild card becomes the seven, then the six, and so on. The first player to work all the way down to zero wins the whole game outright, no scoring required. Special powers are usually layered on top: jacks skip the next player, twos force a draw of two, and the queen of spades makes the next player draw five.
